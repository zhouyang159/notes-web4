...
const Detail = (props) => {
  ...
  const patchNoteMutation = useMutation(
    ...
    },
    {
      ...
      onSuccess: (data, variables, context) => {
        ...
        let {data: encryptStr, msg, status} = data;

        let noteArr = JSON.parse(decryptText(encryptStr));
  ...
  const highLightSearchStr = (searchStr) => {
    if (!searchStr) {
  ...
  const unlockNote = () => {
    if (!notePassword) {
      message.error({content: "password is required!", duration: 3});
      return;
    }
    ...
    axios
      .post(`/user/validateNotePassword/${notePassword.trim()}`)
      .then((res) => {
        if (res.status === 0) {
          ...
          extra={
            <div>
              <Input
                ...
                onChange={(e) => {
                  setNotePassword(e.target.value.trim());
                }}
              ></Input>
              <UnlockBtn
                size="small"
